...
//
// Two source families:
//   * raw 8-bit grayscale asset bitmaps (values 0-255)  -> *Gray8 helpers (>>4)
//   * offscreen ICanvas<Pixel4> buffers                 -> *Canvas helpers
//
// Moved in from Eisei's ui/utils/Blit.hpp as a pure move (unwn #165): bodies
// unchanged, EiseiCanvas generalized to ICanvas<Pixel4>.
...
namespace enjin2
{
                          ...
                          const uint8_t *src, int16_t w, int16_t h)
    {
        for (int16_t py = 0; py < h; ++py)
            for (int16_t px = 0; px < w; ++px)
                dst.setPixel(x + px, y + py, Pixel4(src[py * w + px] >> 4));
    }
                               ...
                               int16_t w, int16_t h)
    {
        for (int16_t py = 0; py < h; ++py)
            for (int16_t px = 0; px < w; ++px)
            {
                ...
                if (g == matte)
                    continue;
                dst.setPixel(x + px, y + py, Pixel4(g >> 4));
            }
    }
        ...
        for (int16_t py = 0; py < h; ++py)
            for (int16_t px = 0; px < w; ++px)
            {
                ...
                if (g == matte)
                    continue;
                const int s = g >> 4;
                dst.setPixel(x + px, y + py, Pixel4(uint8_t(s / divisor)));
            }
    }
    ...
    inline void addGray8(ICanvas<Pixel4> &dst, const uint8_t *src, int16_t w, int16_t h)
    {
        for (int16_t py = 0; py < h; ++py)
            for (int16_t px = 0; px < w; ++px)
            {
                const int s = src[py * w + px] >> 4;
                const int d = dst.getPixel(px, py);
                dst.setPixel(px, py, Pixel4(uint8_t(std::min(15, d + s))));
            }
    }
                                ...
                                const uint8_t *src, int16_t w, int16_t h)
    {
        for (int16_t py = 0; py < h; ++py)
            for (int16_t px = 0; px < w; ++px)
            {
                ...
                dst.setPixel(x + px, y + py, Pixel4(uint8_t(std::abs(d - s))));
            }
    }
                          ...
                          int16_t x = 0, int16_t y = 0)
    {
        for (int16_t sy = 0; sy < static_cast<int16_t>(src.getHeight()); ++sy)
            for (int16_t sx = 0; sx < static_cast<int16_t>(src.getWidth()); ++sx)
            {
                ...
                dst.setPixel(x + sx, y + sy, Pixel4(uint8_t(std::min(15, d + s))));
            }
    }
