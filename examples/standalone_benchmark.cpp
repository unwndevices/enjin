/**
 * @file standalone_benchmark.cpp
 * @brief Self-contained benchmark: Enjin2 vs simulated Adafruit-GFX
 * 
 * Uses simplified Adafruit-style API to compare performance
 */
...
/**
 * @brief Simplified GFXcanvas8 implementation for benchmarking
 */
class GFXcanvas8 {
private:
...
public:
    GFXcanvas8(uint16_t w, uint16_t h) : _width(w), _height(h) {
        buffer.resize(w * h, 0);
    }
...
private:
    ...
    size_t calculateMemoryUsage() {
        ...
        return canvas.getBufferSize() + sizeof(BenchmarkConfig) + 256; // ~8.3KB total
    }
};
 ...
 */
int main() {
    std::cout << "=== ENJIN2 vs ADAFRUIT-GFX STANDALONE BENCHMARK ===" << std::endl;
    std::cout << "Comparing orbital animation performance..." << std::endl;
